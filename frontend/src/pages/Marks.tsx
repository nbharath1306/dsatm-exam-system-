            ...
            <tr>
              ...
              return (
                <div className="min-h-screen bg-gradient-to-br from-blue-900 via-indigo-900 to-purple-900 flex flex-col items-center py-12 px-4">
                  <div className="w-full max-w-4xl bg-white/10 backdrop-blur-lg rounded-2xl shadow-2xl p-8 animate-fade-in">
                    <div className="flex justify-between items-center mb-6">
                      <h2 className="text-3xl font-bold text-white text-center">Marks</h2>
                      <button
                        className="px-4 py-2 rounded bg-green-500 hover:bg-green-600 text-white font-semibold shadow"
                        onClick={exportPDF}
                      >Export PDF</button>
                    ...
                    <table className="w-full text-left text-indigo-100">
                      <thead>
                        <tr>
                          ...
                          <th className="pb-2">Theory Marks</th>
                          <th className="pb-2">Practical Marks</th>
                      ...
                      <tbody>
                        {filtered.map((m: Mark) => (
                          <tr key={m.id} className="hover:bg-indigo-900/30 transition-colors">
                            <td className="py-2">{m.student?.name} ({m.student?.usn})</td>
                            <td>{m.subject?.name} ({m.subject?.code})</td>
                            <td>{m.subject?.category}</td>
                            <td>{m.theoryMarks ?? '-'}</td>
                            <td>{m.subject?.category === 'IPCC' ? (m.practicalMarks ?? '-') : '-'}</td>
                            <td>
                              <button
                  ...
                  <style>{`
                    ...
                    .animate-scale-in { animation: scaleIn 0.3s; }
